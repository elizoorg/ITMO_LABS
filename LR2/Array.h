...
public:
	...
	Array(Array&& toMove) noexcept {
		std::cout << std::format("Move constr \n");
		std::swap(lastitem, toMove.lastitem);
		std::swap(_size, toMove._size);
		std::swap(items, toMove.items);
	}
	...
	public:
		...
		Iterator& operator++() {
			std::cout << std::format("Iterator operator++ \n");
			++ptr_;
			return *this;
		}
		Iterator operator++(int) {
			std::cout << std::format("Iterator operator++(int) \n");
			Iterator copy{ *this };
			++ptr_;
			return copy;
		}
		...
		const T& get() const { std::cout << std::format("Iterator get() \n"); return *ptr_; }
	...
	public:
		...
		const T get() { std::cout << std::format("4LEN\n"); return *it_; }
	...
	Iterator reverseIterator() { std::cout << lastitem << std::endl; 
	Iterator it(items + lastitem-1);
	it.reverse();
	return it;
	}
	...
	Array(std::initializer_list<T> l) : Array<T>(l.size()) {
		std::cout << std::format("Construct from std::initializer_list \n");
